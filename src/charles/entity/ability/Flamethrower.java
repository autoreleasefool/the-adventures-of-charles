...
public class Flamethrower extends Ability {
    ...
    public Flamethrower (Engine engine, int x, int y, int attack, int direction) {
	//Sends these values to the super class
	super(engine, x, y, 256, 64, attack, 1, 1, direction, false, true);
    }
	...
	if (moveTimer > 0) {
		...
		if (targets[e] != null) //If the target was not killed
		    ...
		    g.drawImage(getImage(), targets[e].getX() + targets[e].getWidth() / 2 - width / 2, targets[e].getY() + targets[e].getHeight() - height, null);
	    }
	}
    }
	...
	return (Image) (Art.abilitySheet.getSubimage(160 + (frame / 5) * 32 + 64 * direction, 160, 32, 32));
    }
	...
	if (moveTimer > 0) {    //If the animation is moving
		...
		if ((x + 28 > targets[0].getX() + targets[0].getWidth() / 2 && direction == 0) || (x + 4 < targets[0].getX() + targets[0].getWidth() / 2 && direction == 1) || moveTimer == 0) {
		    ...
		    targets[0].takeDamage(attack, source);  //Deals damage to enem
		}
	    } else  //If the enemies are all gone this ability expires
		expire();  //Exits the method
	    return;
	...
	for (int xx = 0; xx<width; xx+=width / 5) {         //Checks field bit by bit to hit closest enemy
		...
		if (new Rectangle(x + ((direction == 0) ? xx:-xx), y, width / 5, height).intersects(enemies[e].getRectangle())) {
		    //Moves the ability to the enemy's y
		    y = enemies[e].getY() + enemies[e].getHeight() / 2 - 16;
