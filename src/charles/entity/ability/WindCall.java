...
public class WindCall extends Ability {

    public WindCall(Engine engine, int x, int y, int time) {
	//Sends these values to the super class
	super(engine, x, y, 16, 64, 0, time, 0, 0, false, false);
    }
	...
	//If the time is at less than 30 frames
	if (animFrame < 30)
	...
	return (Image)(Art.abilitySheet.getSubimage(animFrame / 6 * 16, 0, 16, 64));
    }
    
    //Returns "active ability" icon of the ability from the spritesheet
    private Image getPassiveImage() {
	return (Image)(Art.abilitySheet.getSubimage(80, 0, 32, 32));
    }
}
